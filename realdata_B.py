...
def manhattan(samples, clusters, k, cutoff):
    n_loop = 0
    while True:
        #print(clusters)
        spare_clusters = np.empty(shape=[0, 2])
...
#Optimizing the center points with Manhattan
def hod_mann_hl1(samples, clusters, k, cutoff):
    n_loop = 0
    while True:
        #print(clusters)
        spare_clusters = np.empty(shape=[0, 2])
...
def hod_mann_hl2(samples, clusters, k, cutoff):
    n_loop = 0
    while True:
        #print(clusters)
        spare_clusters = np.empty(shape=[0, 2])
...
def hod_mann_hl3(samples, clusters, k, cutoff):
    n_loop = 0
    while True:
        #print(clusters)
        spare_clusters = np.empty(shape=[0, 2])
...
if __name__ == '__main__':
    ...
    for i in range(10000):
        ...
        for i in range(len(data)):
            #data[i] = data[i] + np.random.rand()-0.5
            #data[i] = list(machine_noise(data[i]))[0]
        ...
        n_clusters_means, coor_xy_means, coor_w_means= kmeans(x_y_w, clusters, k, 0.00000001)
        no_conta.append(n_clusters_means)
        n_clusters_means_copy, _, _= kmeans(x_y_w_copy, clusters, k, 0.00000001)
        ...
        n_clusters_medians, coor_xy_medians, coor_w_medians= manhattan(x_y_w, clusters, k, 0.00000001)
        no_conta.append(n_clusters_medians)
        #print(n_clusters_medians)
        n_clusters_medians_copy, _, _= manhattan(x_y_w_copy, clusters, k, 0.00000001)
        ...
        n_clusters_bfgs, coor_xy_bfgs, coor_w_bfgs= bdfs_l2(x_y_w, clusters, k, 0.00000001)
        no_conta.append(n_clusters_bfgs)
        #print(n_clusters_bfgs)
        n_clusters_bfgs_copy, _, _= bdfs_l2(x_y_w_copy, clusters, k, 0.00000001)
        ...
        n_clusters_hl1_copy, _, _ = hod_mann_hl1(x_y_w_copy, clusters, k, 0.00000001)
